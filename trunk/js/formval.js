                         ...
                         required)   // true if required
{
  ...
  var email = /^[^@]+@[^@.]+\.[^@]*\w\w$/  ;
  if (!email.test(tfld)) {
    msg (infofield, "error", "ERROR: no es una direccion de e-mail valida");
    setfocus(valfield);
    return false;
  }
                         ...
                         required)   // true if required
{
  ...
  var telnr = /^\+?[0-9 ()-]+[0-9]$/  ;
  if (!telnr.test(tfld)) {
    msg (infofield, "error", "ERROR: no es un numero de telefono valido. Caracteres permitidos: digitos, espacio ()- y +");
    setfocus(valfield);
    return false;
  }
  ...
  if (numdigits<6) {
    msg (infofield, "error", "ERROR: " + numdigits + " digitos - muy corto");
    setfocus(valfield);
    return false;
  }
  ...
  if (numdigits>14)
    ...
    if (numdigits<10)
      msg (infofield, "warn", "Solo " + numdigits + " digitos - verifique si es correcto");
    else
      msg (infofield, "warn", "");
  }
  return true;
						 ...
						 maxval)   // allowed maximum value
{
	...
	if (!ageRE.test(tfld)) {
    	msg (infofield, "error", "ERROR: no es un valor valido");
	    setfocus(valfield);
    	return false;
  	}

  if (tfld>maxval){
	  msg (infofield, "warn", "Valor mayor al limite maximo: "+maxval);
	  setfocus(valfield);
	  return false;
	}else{
		if (tfld<minval){
			msg (infofield, "warn", "Valor menor al limite minimo: "+minval);
			setfocus(valfield);
			return false;
                                    ...
                                    //             1 = greater than or equal to nr
{
  ...
  if (count<nr) errorMsg = 'Solo ' + count + ' radio' + suffix + ' marcado: ' + nr + ' requerido';
  if (count>nr) errorMsg = '' + count + ' radio marcado: maximo ' + nr + ' permitido';
  if (count==0) errorMsg = 'No hay opcion seleccionada: ' + nr + ' requerido'; //modified for radio buttons
                         ...
                         required)   // true if required
{
  ...
  var ageRE = /^[0-9]/
  if (!ageRE.test(tfld)) {
    msg (infofield, "error", "ERROR: no es un numero valido");
    setfocus(valfield);
    return false;
  }
